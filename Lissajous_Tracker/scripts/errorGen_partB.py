...
from week2_203230010_193236001.msg import Error_data
import time
...
def programflow():
	...
	x, y = waypoints(4,1,2); # waypoints

	#Loop Variables
	z = 0

	#Co-ordinates and Errors
	E_pos = [None] *1000 	#Array to store the error in robot's position
	E_theta = [None] * 1000 #Array to store the error in robot's heading angle
	graph_x = [None]*1000 	#Array to store the robot's present x co-ordinate
	graph_y = [None]*1000	#Array to store the robot's present y co-ordinate
	...
	while not rospy.is_shutdown() and z<=750: 
		...
		z = z+1 #Incrementing the loop variable
